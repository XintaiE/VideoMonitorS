...
void Camera::setShape(float viewAngle, float aspect, float Near, float far)
{
    ...
    glLoadIdentity();//设置当前矩阵为投影矩阵并归一化
    gluPerspective(viewAngle, aspect, Near, far);//对投影矩阵进行透视变幻
}
...
void Camera::slide(float du, float dv, float dn)
{
    ...
    m_target(1) = m_target(1) + du*u.y() +dv*v.y() + dn*n.y();
    m_target(2) = m_target(2) + du*u.z() +dv*v.z() + dn*n.z();
